...
MenuItem *menuItems[] = {
  ...
  ///Расчет слоев
  new MenuItem(0, 4, PSTR("Расчет слоев")),
  ...
  if (encoder.click()) {
    switch (menu.index)  // Если было нажатие, то выполняем действие, соответствующее текущей позиции курсора
    {
      ...
      case miSettingsBack:
        ...
        // Новый case для расчета слоев  
        case MENU_LAYER_CALC: {  
        LayerCalculator calc(lcd, params[currentWinding]);  
        calc.showMenuItem();  
        ...
        while (!exit) {  
            ...
            if (encoder.turn()) {  
                if (encoder.dir() > 0) {  
                    exit = calc.handleButton(BTN_DOWN);  
                } else {  
                    exit = calc.handleButton(BTN_UP);  
                }  
            }  
            
            if (encoder.turnH()) {  
                if (encoder.dir() > 0) {  
                    exit = calc.handleButton(BTN_RIGHT);  
                } else {  
                    exit = calc.handleButton(BTN_LEFT);  
                }  
            }  
            
            if (encoder.click()) {  
                exit = calc.handleButton(BTN_OK);  
            }  
